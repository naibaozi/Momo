...
public class UserinfoServlet extends HttpServlet {
    ...
    private void bindWechat(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        if (code == null || code.trim().isEmpty()) {
            sendErrorResponse(out, 400, "微信code不能为空");
            return;
        }
        ...
        try {
            ...
            if (wechatResp.containsKey("errCode")) {
                ...
                // 直接返回微信的错误信息，便于定位问题
                sendErrorResponse(out, 400, "微信接口错误: " + errCode + " - " + errMsg);
                return;
            }

            String openid = wechatResp.getString("openid");
            if (openid == null || openid.trim().isEmpty()) {
                sendErrorResponse(out, 400, "未获取到openid，微信返回: " + wechatRespStr);
                return;
            }
            ...
            result.put("data", Map.of(
                    ...
                    "avatarUrl", updateUser.getAvatar()
            ));
            ...
            e.printStackTrace();
            sendErrorResponse(out, 500, "绑定失败：" + e.getMessage());
        }
    }
    ...
    private void wechatQuickLogin(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            if (user == null) {
                sendErrorResponse(out, 404, "该微信未绑定账号，请先绑定");
                return;
            }
            ...
            result.put("token", token); // 前端存储token用于后续请求
            result.put("data", user); // 返回用户信息（不含密码）
            out.write(JSON.toJSONString(result));

        } catch (Exception e) {
            e.printStackTrace();
            sendErrorResponse(out, 500, "登录失败：" + e.getMessage());
        }
    }
    ...
    private void sendEmailCode(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        if (email == null || email.trim().isEmpty() || !Pattern.matches(EMAIL_REGEX, email.trim())) {
            sendErrorResponse(out, 400, "请输入有效的邮箱地址");
            return;
        }
        ...
        if (!verificationCodeService.canSendNewCode(email)) {
            sendErrorResponse(out, 400, "验证码发送过于频繁，请1分钟后再试");
            return;
        }
        ...
        if (sendSuccess) {
            ...
            Map<String, Object> result = new HashMap<>();
            result.put("code", 200);
            result.put("message", "验证码已发送至您的邮箱，有效期5分钟");
            out.write(JSON.toJSONString(result));
        } else {
            sendErrorResponse(out, 500, "验证码发送失败，请稍后重试");
        }
    }
    ...
    private void verifyEmailCode(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        if (email == null || email.trim().isEmpty() || code == null || code.trim().isEmpty()) {
            sendErrorResponse(out, 400, "邮箱和验证码不能为空");
            return;
        }
        ...
        if (isValid) {
            ...
            sendErrorResponse(out, 400, "验证码无效或已过期");
        }
    }
    ...
    private void addUserinfo(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        if (userByEmail != null) {
            sendErrorResponse(out, 400, "该邮箱已被注册");
            return;
        }
        ...
        String plainPwd = cryptoUtil.aesDecrypt(encryptedPwd);
        if (plainPwd == null) {
            sendErrorResponse(out, 400, "密码解密失败");
            return;
        }

        // 7. 后端密码复杂度校验
        if (!isValidPassword(plainPwd)) {
            sendErrorResponse(out, 400, "密码不符合要求：至少8位，包含字母和数字");
            return;
        }
        ...
        if (newId > 0) {
            ...
            result.put("data", Map.of("userId", newId));
            out.write(JSON.toJSONString(result));
        } else {
            sendErrorResponse(out, 500, "注册失败");
        }
    }
    ...
    private void updateUserinfo(HttpServletRequest request, HttpServletResponse response, PrintWriter out) {
        ...
        try {
            ...
            if (currentUserId == null) {
                sendErrorResponse(out, 401, "未登录，请重新登录");
                return;
            }
            ...
            if (newEmail != null && !newEmail.trim().isEmpty()
                ...
                if (emailCode == null || emailCode.trim().isEmpty()) {
                    sendErrorResponse(out, 400, "请输入邮箱验证码");
                    return;
                }
            ...
            if (rows > 0) {
                ...
                result.put("data", userDao.getUserById(updateId)); // 返回更新后的完整用户信息
                out.write(JSON.toJSONString(result));
            } else {
                sendErrorResponse(out, 500, "更新失败，未修改任何内容");
            }

        } catch (NumberFormatException e) {
            System.err.println("ID格式错误: " + e.getMessage());
            sendErrorResponse(out, 400, "用户ID必须为数字");
        } catch (Exception e) {
